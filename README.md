...
flowchart TD
    ...
    R3 -->|仅节点| R5[准备仅节点输出]
    R4 --> R6[下载 Xray / 配置 UFW / 安装并验收 Xray]
    R5 --> R6
    R6 --> R7{执行已选订阅分支}
    R7 -->|部署| R8[安装 Nginx / 申请订阅证书 / 生成并验收订阅]
    R7 -->|仅节点| R9[跳过订阅服务]
...
图中是安装器的实际执行顺序。Reality 在选择订阅输出后立即进入对应的参数分支，再汇合安装公共 Xray 运行时；订阅分支部署完成后才统一保存最终状态、注册命令和配置配额任务。CDN XHTTP 则先创建 Route 53 源站记录，再配置本机源站，最后创建并验收 CloudFront。
