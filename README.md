...
Cloudflare Free Zone 的基础额度按 Provider 当前规则执行，域名注册费和 VPS 费用另计。
...
| --- | --- | --- |
| **Windows** | [Clash Verge Rev](https://github.com/clash-verge-rev/clash-verge-rev/releases)<br>[Mihomo Party](https://github.com/mihomo-party-org/clash-party/releases)<br>[FlClash](https://github.com/chen08209/FlClash/releases) | **Clash Verge Rev**（主流首选）：UI 现代、开箱即用，原生内置最新 Mihomo 内核，系统代理与 TUN 模式非常稳定。<br>**Mihomo Party**：跨平台高颜值、活跃更新维护，图形化路由与订阅管理出色。<br>**FlClash**：基于 Flutter，跨平台极简，内存占用极低。<br>**导入方式**：在“订阅 / 订阅管理”中新建订阅，粘贴输出的 Mihomo 地址，保存并启用。 |
| **macOS** | [Clash Verge Rev](https://github.com/clash-verge-rev/clash-verge-rev/releases)<br>[Mihomo Party](https://github.com/mihomo-party-org/clash-party/releases)<br>[FlClash](https://github.com/chen08209/FlClash/releases) | **Clash Verge Rev**：原生适配 Apple Silicon (M 系列芯片) 与 Intel，菜单栏常驻与全局路由切换体验极佳。<br>**Mihomo Party**：现代化跨平台体验，支持深色模式与灵活分流配置。<br>**FlClash**：轻量清爽，操作逻辑一致。<br>**导入方式**：在订阅设置中粘贴带 `flag=clash` 的 URL，启用系统代理或 TUN。 |
...
| **iOS / iPadOS** | [Clash Mi 官方下载页](https://clashmi.app/download)<br>[Clash Hako (App Store)](https://apps.apple.com/app/id6794257189)<br>[Shadowrocket（小火箭）](https://apps.apple.com/app/shadowrocket/id932747118) | **Clash Mi**（首选推荐）：App Store / TestFlight 可用，专为 Mihomo / Clash Meta 打造，完整支持 XHTTP stream-up 与一键配置导入；配图说明详见其[官方用户手册](https://clashmi.app/guide/)。<br>**Clash Hako**（[官网](https://clash.md) / [App Store](https://apps.apple.com/app/id6794257189)）：基于 Mihomo (Hako) 内核开发，采用原生 SwiftUI 构建，支持 iOS/iPadOS/macOS。**注：该客户端最近才发布，目前尚处于早期迭代阶段**，功能与连接稳定性仍在持续更新演进中。<br>**Shadowrocket**：**不作为本项目首选推荐**。虽支持 Base64 / 单节点，但对精选 IP 所需的 IP/SNI/Host 分离解析能力需自行逐节点测试确认。 |
| **Linux** | [Clash Verge Rev](https://github.com/clash-verge-rev/clash-verge-rev/releases)<br>[Mihomo Party](https://github.com/mihomo-party-org/clash-party/releases)<br>[FlClash](https://github.com/chen08209/FlClash/releases)<br>[Mihomo Core (CLI)](https://github.com/MetaCubeX/mihomo/releases) | **桌面环境**：推荐使用 Clash Verge Rev、Mihomo Party（提供 AppImage/deb/rpm）或 FlClash。<br>**服务器/路由器环境**：可直接下载原生 Mihomo 二进制核心配合 systemd 守护进程运行。 |
...
flowchart TD
    ...
    B -->|5| C0[Cloudflare CDN 精选 IP 纯 XHTTP stream-up]
    C0 --> C1[系统预检 / 冲突检查 / 备份]
    ...
    C3 --> C4[官方高优 CIDR 权重抽样 / 全网预筛 + TLS 验证 / 筛选 Top 5 精选 IP]
    C4 --> C5[保存缓存 / 注册每小时刷新 / 输出纯 XHTTP 5 节点与订阅]
    C5 --> Z
```

图中是安装器的实际执行顺序。两种模式都只询问一次订阅输出；后续步骤只应用已保存的选择，不会再次询问。部署 CDN 订阅时可直接复用节点域名，也可输入独立的完整订阅域名。Cloudflare 模式只使用单一 proxied 一级子域，由 VPS 预筛并由客户端最终测速选优。
...
- **边缘规则与安全防护**：
  ...
  - 边缘开启 HTTP/2 与 gRPC（部署前必须在目标 Zone 的 **Network → gRPC** 中手动开启 gRPC；该开关没有可用 API。安装、`apply-cloud` 和 `refresh-cdn-ips` 会执行边缘验收，发现 Cloudflare 返回 `403` 时立即停止）。
...
- **定时刷新与客户端测速**：
  - VPS 使用 systemd timer 每小时更新缓存；安装、`apply` 和手动 `refresh-cdn-ips` 都会自动修复并验收该 timer。刷新失败时保留旧缓存；未生成缓存或缓存超过 24 小时继续使用现有缓存。
  - Mihomo 每 300 秒在客户端网络运行一次 `url-test` 自动选优。

### 精选 IP 的客户端要求

本项目的精选 IP 订阅按 Mihomo 的配置格式和 XHTTP 能力生成，需要使用 Mihomo，或明确兼容同等 Mihomo XHTTP 字段的客户端。它不是把节点域名简单替换成 IP：每个 IP 节点的 `server` 是筛选出的 Cloudflare IPv4，但 `servername` 和 `xhttp-opts.host` 仍然必须是节点域名，同时依赖 XHTTP `stream-up`、路径以及独立的 TLS SNI/HTTP Host 字段。客户端如果不能分别保存 IP、TLS SNI 和 HTTP Host，精选 IP 节点会连接失败；客户端是否兼容，最终仍需以实际生成订阅的导入测试为准。
...
该模式只使用一枚限制到目标 Zone 的 API Token（Zone Read、DNS Edit、Transform Rules Edit、Config Rules Edit、Zone Settings Edit、SSL and Certificates Edit）。完整的 DNS、证书、规则、防火墙和条款/100 MB/长连接风险说明见 [前置准备手册](docs/preparation-guide.md)。
...
STATE_VERSION=7  # Cloudflare 纯 XHTTP stream-up
...
Cloudflare 订阅 A 记录、Strict TLS 规则和 Origin CA；Cloudflare 模式清理其受管 DNS、规则和 Origin CA。
...
│  └─ xhttp-cloudflare-streamup.sh  Cloudflare 纯 XHTTP stream-up Provider、状态与安装编排
├─ lib/
│  ├─ xhttp-runtime.sh             CDN Profile 复用的本机运行时骨架
│  ├─ globalping-cdn.sh            Cloudflare 精选 IPv4、缓存与每小时刷新任务
│  ├─ cloudflare-ip-pool.sh        Cloudflare 官方 IPv4 池抽样与候选筛选
│  ├─ quota.sh                     用户配额与统计
...
入口负责模式选择、命令分发和完整运行时的原子注册。Reality 与 Cloudflare
...
Globalping 严格零丢包筛选、用户凭据与月度配额、TCP 参数回滚、Xray 配置、
