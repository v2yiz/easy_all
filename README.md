...
| 3. AWS CDN - XHTTP | VLESS XHTTP stream-up / HTTP2 | Route 53 + ACM + CloudFront |
| 4. Gcore CDN 精选 IP - XHTTP | VLESS XHTTP stream-up / HTTP2 | Gcore CDN + Globalping IPv4 |
| 5. AWS CDN 精选 IP - XHTTP | VLESS XHTTP stream-up / HTTP2 | CloudFront + Globalping IPv4 |

四个 CDN XHTTP 模式复用同一套 Xray、Nginx、订阅与用户配额运行时。精选 IP 模式由 VPS
Globalping 预筛、客户端测速选优；精选 IP 只改变客户端入口，不创建第二套 CDN 资源。

同一台 VPS 只能安装一种模式。脚本会管理 Xray、Nginx、证书、UFW、BBR 和订阅文件，
只适合专用 VPS。

五种安装模式都会保留 sshd 已检测到的现有端口，并通过公共平台模块额外监听 TCP `65533`；
UFW 会在拒绝其他入站流量前同时放行现有 SSH 端口和 `65533`。安装与 `easy_all apply`
都会校验 sshd 配置、实际监听套接字和 UFW 规则，任一环节失败都会停止应用。五种模式还会
...
请选择安装模式：
  ...
  3. AWS CDN - XHTTP（非优化线路推荐）
  4. Gcore CDN 精选 IP - XHTTP（中国大陆 Globalping 预筛 + 客户端测速）
  5. AWS CDN 精选 IP - XHTTP（中国大陆 Globalping 预筛 + 客户端测速）
请选择 [1]（直接回车使用默认值）:
```
...
flowchart TD
    ...
    B -->|4| G0[Gcore CDN 精选 IP XHTTP]
    G0 --> G1[系统预检 / 冲突检查 / 备份]
    G1 --> G2[依赖 / SSH 启动保障 / XanMod LTS BBRv3 / 重启策略]
    G2 --> G3[Gcore 源站域名 / CDN 域名 / Globalping Token / VLESS 自动参数]
    G3 --> G4{订阅输出选择}
    ...
    G9 --> G10[源组 / Gcore CDN / CNAME / 免费 Let's Encrypt / Globalping 精选 IPv4 / 生成订阅]
    G10 --> G11[保存状态 / 注册 easy_all / 配置用户配额、980 GB 保护与每小时刷新]
    G11 --> Z

    B -->|5| O0[AWS CDN 精选 IP XHTTP]
    O0 --> O1[执行与模式 3 相同的 AWS CloudFront 安装]
...
图中是安装器的实际执行顺序。五种模式都只询问一次订阅输出；后续步骤只应用已保存的选择，不会再次询问。部署 CDN 订阅时可直接复用节点域名，也可输入独立的完整订阅域名。Cloudflare 模式只使用单一 proxied 一级子域；AWS 只接受 Route 53 Public Hosted Zone，Gcore 只接受 Gcore Managed DNS Zone。所有精选 IP 模式均由 VPS 预筛并由客户端最终测速选优。
...
| Globalping Token | 模式 2/4/5 必填，隐藏输入 | 无 | 不允许为空；保存到 root-only 独立文件 |
...
| 安装模式 | `1` Reality / `2` Cloudflare 精选 IP / `3` AWS CDN XHTTP / `4` Gcore 精选 IP / `5` AWS 精选 IP | `1` | 安装 Reality |
...
五种安装模式的 Xray 普通公网出站统一使用 `AsIs`，由系统拨号器自动处理 IPv4/IPv6。Gemini 页面、
...
服务器把启用 `SO_KEEPALIVE` 的 TCP 套接字默认探测参数设为 `300/30/5`，并在五种 Xray 入站
...
五种模式统一安装 XanMod LTS 内核；XanMod 官方将 Google BBRv3 内置为默认 `tcp_bbr`，因此
...
| CDN XHTTP（Cloudflare/AWS/Gcore） | 1. 读取状态，备份 Xray/Nginx 配置和订阅文件。<br>2. 安装或验收 XanMod LTS BBRv3、重写 TCP 参数，并按当前状态同步 SSH 监听、UFW 与 Fail2ban。<br>3. 重启前结算尚未写入账本的 Xray 流量，再生成并验收 Xray 与 Nginx。<br>4. 按已保存的选择重建并验收订阅，或删除订阅文件；精选 IP 模式使用现有 Globalping 缓存。<br>5. 保存状态、注册当前代码、恢复用户配额、全局费用保护及 Globalping 刷新任务并显示输出。普通 `apply` 不读取云端凭证、不修改云资源。 |
...
模式 3 只输出一个以 CDN 域名连接的 VLESS XHTTP 节点；模式 5 使用相同的 CloudFront
...
| 客户端节点 IP 族 | 模式 3 为 `ipv6-prefer`；模式 2/4/5 为 `ipv4` | 自动固定，无需输入 |
...
### 模式 2/4/5：Globalping 精选 IPv4

模式 5 不创建另一套 AWS 资源。CloudFront、ACM、Route 53、源站证书、UUID、XHTTP 路径和
Origin Key 都与模式 3 相同，区别只在客户端入口：
...
订阅响应设置 `Cache-Control: no-store`。模式 3 的客户端节点和所有订阅地址使用 CDN 域名；
模式 5 的节点连接地址使用精选 IPv4，但 SNI/Host 仍使用 CDN 域名。两种模式都不会暴露源站域名。
...
Gcore 是菜单中的第四项：它复用 XHTTP 本机运行时，但只使用 `GCORE_API_TOKEN` 管理 Gcore
...
AWS_CDN_ENDPOINT_MODE=domain|optimized        # AWS 模式 3/5
GCORE_CDN_ENDPOINT_MODE=optimized              # Gcore 模式 4
CDN_CLIENT_IP_FAMILY=ipv6-prefer|ipv4         # 模式 2/4/5 固定 IPv4，模式 3 为 IPv6 优先
...
Token、`GCORE_API_TOKEN` 或 Globalping Token 持久化到状态文件。模式 2/4/5 的 Globalping Token
...
五种安装模式的 IPv4 直连、私网目标阻断、Reality 客户端地址族策略、Globalping 严格零丢包筛选、
Cloudflare/AWS/Gcore XHTTP、用户凭据与月度配额、TCP 参数回滚、Xray 配置、CloudFront JSON、Gcore API
