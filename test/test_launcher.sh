...
assert_equal "newer Reality state selects Reality profile" "reality" "$(detect_installed_mode)"
...
printf 'STATE_VERSION=10\nPROTOCOL=cloudflare-streamup\nCDN_PROVIDER=cloudflare\n' >"${EASY_ALL_STATE_FILE}"
assert_equal "newer Cloudflare state selects cloudflare-streamup" "cloudflare-streamup" "$(detect_installed_mode)"
