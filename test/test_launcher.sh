...
git() {
    ...
    mkdir -p "${destination}"
    for relative_path in easy_all aggregate/setup.sh aggregate/aggregate.example.json aggregate/README.md templates/mihomo.yaml aggregate/aggregate.mjs worker-src/index.js aggregate/easy_all-aggregate.service "${EASY_ALL_RUNTIME_MODULES[@]}"; do
...
[[ "${guide}" == *"[1 默认] 直连 Reality"* \
    ...
    && "${guide}" == *"[3] Gcore CDN 精选 IP - 优质单播节点"* \
    && "${guide}" == *"全网综合优选"* \
    && "${guide}" == *"定向不交叉测速"* \
    && "${guide}" == *"XHTTP"* \
    && "${guide}" != *"AWS"* ]] \
