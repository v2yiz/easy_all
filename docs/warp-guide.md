...
WireGuard 出站，并且只把 Gemini/AI Studio 的最小精确域名集合导向 WARP，包括 Gemini
本体、Gemini 静态资源和 Gemini/AI Studio API 端点；不会修改 VPS 系统默认路由，也不会把
Google 登录、Google 首页、通用 gstatic、通用 googleapis 或通用 clients 域名切到 WARP。
