...
Gemini 静态资源、Gemini/AI Studio API 端点，以及 `waa-pa.clients6.google.com` 和
`signaler-pa.clients6.google.com`；不会修改 VPS 系统默认路由，也不会把 Google 登录、
Google 首页、通用 gstatic、通用 googleapis 或其他通用 clients 域名切到 WARP。
