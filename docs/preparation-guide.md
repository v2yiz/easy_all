...
先确定只安装其中一种链路，再准备对应资源。同一台 VPS 只能安装一种模式。
...
- **真实入口发现**：默认通过 Globalping 从香港、日本、韩国、新加坡、洛杉矶、圣何塞以及中国移动、
  联通、电信网络解析 `VLESS_CDN_DOMAIN`；香港、日本、洛杉矶还会分别使用探针默认 DNS、`1.1.1.1`
  和 `8.8.8.8`。核心地区默认各取 8 个探针，合并成功响应中的公共 IPv4 并去重。
