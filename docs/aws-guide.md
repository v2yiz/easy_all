...
费用提示：默认按量模式在适用的 CloudFront 免费额度内，常规总费用预期约 **$0.60/月**，主要是
一个 Route 53 Public Hosted Zone 的 `$0.50/月` 加少量标准 DNS 查询。超出免费额度、使用第二个
Hosted Zone 或启用其他资源后会增加；请按本文第 2 节的边界设置费用提醒并启用脚本的流量保护。
...
| 按量付费（脚本默认） | 每月免费 **1 TB + 1000 万次请求**。脚本按 UTC 自然月累计 Xray 上下行总流量并在 **980 GB** 阻断；免费额度内常规总费用预期约 **$0.60/月**。超过 1 TB 后按边缘区域计价，每多 100 GB 约 `$8.50-$12.00`，超额请求另计。 | 不创建 WAF。每个 Public Hosted Zone 约 `$0.50/月`；CloudFront Alias A/AAAA 查询免费，其他标准查询约 `$0.04/10万次`、`$0.40/百万次`。一个 Zone 加少量普通查询常规约 `$0.60/月`，两个 Zone 通常约 `$1.00/月` 以上。 |
