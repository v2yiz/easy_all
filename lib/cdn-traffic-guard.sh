...
# Gcore CDN global traffic safety guard.
...
configure_cdn_traffic_protection() {
    if [[ -z "${CDN_TRAFFIC_PROTECTION_GB:-}" \
        ...
        CDN_TRAFFIC_PROTECTION_GB=${DEFAULT_GCORE_CDN_TRAFFIC_PROTECTION_GB}
    fi
    validate_cdn_traffic_protection_gb "${CDN_TRAFFIC_PROTECTION_GB}" \
        || die "Gcore CDN 全局费用保护额度必须是 1-1000 的整数 GB"
    CDN_TRAFFIC_PROTECTION_GB=$((10#${CDN_TRAFFIC_PROTECTION_GB}))
    [[ "${CDN_TRAFFIC_PROTECTION_GB}" == "${DEFAULT_GCORE_CDN_TRAFFIC_PROTECTION_GB}" ]] \
        || die "Gcore CDN 全局费用保护额度固定为 ${DEFAULT_GCORE_CDN_TRAFFIC_PROTECTION_GB} GB"
}
...
initialize_cdn_traffic_usage() {
    ...
    if [[ -s "${CDN_TRAFFIC_GUARD_USAGE_FILE}" ]]; then
        ...
        validate_cdn_traffic_usage "${usage}" \
            || die "Gcore CDN 全局费用保护统计文件损坏：${CDN_TRAFFIC_GUARD_USAGE_FILE}"
        return 0
    fi
...
[Unit]
Description=easy_all Gcore CDN global traffic protection
After=${XRAY_SERVICE}

[Service]
Type=oneshot
ExecStart=${COMMAND_PATH} cdn-traffic-sync
EOF
...
cdn_traffic_update_usage() {
    ...
    if [[ "$(jq -r '.period' <<<"${usage}")" == "${period}" ]]; then
        ...
        # risking an unmetered gap at the Gcore billing boundary.
...
cdn_traffic_protection_sync() {
    ...
    if [[ "${CDN_TRAFFIC_NEEDS_APPLY}" == "1" ]] \
        ...
        release_runtime_write_lock
        die "应用 Gcore CDN 全局费用保护状态失败；已保留待执行状态并将在下次重试"
    fi
