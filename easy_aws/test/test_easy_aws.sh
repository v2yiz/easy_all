...
(
    ...
    [[ "${link}" != *"xhttp"* && "${link}" != *"grpc"* && "${link}" != *"trojan"* ]] \
        || fail "link must contain only VLESS WebSocket"
    ...
    [[ "${mihomo}" != *"xhttp"* ]] || fail "Mihomo output must not include XHTTP"
    ...
    jq -e '
        ...
        (.DefaultCacheBehavior.AllowedMethods.Items | sort) == (["GET","HEAD"] | sort) and
    ...
    [[ "$(<"${worker}")" != *'"network":"xhttp"'* ]] \
        || fail "rendered Worker must not include XHTTP"
...
assert.doesNotMatch(decoded, /xhttp|trojan|grpc/);
...
assert.doesNotMatch(clash, /network: xhttp/);
NODE
)
...
assert_contains "README documents one node" "${readme}" "只输出一个 VLESS + WebSocket"
