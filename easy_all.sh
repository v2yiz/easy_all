...
write_worker() {
    ...
    reality)
        config_json=$(jq -cn \
            ...
            '{type:$type,security:$security,network:$network,uuid:$uuid,host:$host,name:$name,sni:$sni,pbk:$pbk,sid:$sid,portMode:$port_mode}')
        ;;
    anytls)
        config_json=$(jq -cn \
            --arg type anytls --arg host "${ANYTLS_DOMAIN}" --arg name "${NODE_NAME}" \
            --arg password "${ANYTLS_PASSWORD}" --arg sni "${ANYTLS_DOMAIN}" \
            --arg port_mode "${SUB_PORT_MODE}" \
            '{type:$type,host:$host,name:$name,password:$password,sni:$sni,portMode:$port_mode}')
        ;;
    vless-wss)
        config_json=$(jq -cn \
            --arg type vless --arg security tls --arg network ws \
            --arg uuid "${VLESS_UUID}" --arg host "${VLESS_WSS_DOMAIN}" --arg name "${NODE_NAME}" \
            --arg sni "${VLESS_WSS_DOMAIN}" --arg path "${WS_PATH}" \
            '{type:$type,security:$security,network:$network,uuid:$uuid,host:$host,name:$name,sni:$sni,path:$path,portMode:"443"}')
        ;;
    esac
    {
...
function dynamicPort() {
  ...
  const hour = Math.floor((nowUtc8.getTime() - yearStart) / 3600000);
  return PORT_BASE + hour * PORT_MULTIPLIER + Math.floor(Math.random() * 6) + 1;
}
function port(config) { return config.portMode === 'dynamic' ? dynamicPort() : 443; }
function enc(value) { return encodeURIComponent(String(value)).replace(/[!'()*]/g, c => '%' + c.charCodeAt(0).toString(16)); }
function b64(value) {
...
function link(config) {
  if (config.type === 'anytls') {
    const params = new URLSearchParams({sni: config.sni, insecure: '0'});
    return `anytls://${enc(config.password)}@${config.host}:${port(config)}/?${params}#${enc(config.name)}`;
  }
  const params = new URLSearchParams({
    encryption: 'none', security: config.security, type: config.network,
    sni: config.sni, fp: 'chrome'
  });
  if (config.security === 'reality') {
    params.set('pbk', config.pbk); params.set('sid', config.sid);
    params.set('flow', 'xtls-rprx-vision'); params.set('packetEncoding', 'xudp');
  } else if (config.network === 'ws') {
    params.set('path', config.path); params.set('host', config.host);
  }
  return `vless://${config.uuid}@${config.host}:${port(config)}?${params}#${enc(config.name)}`;
}
function proxyYaml(c) {
  const q = value => JSON.stringify(String(value));
  const common = [`  - name: ${q(c.name)}`, `    type: ${c.type}`, `    server: ${q(c.host)}`, `    port: ${port(c)}`];
  if (c.type === 'anytls') {
    return [...common, `    password: ${q(c.password)}`, '    udp: true', `    sni: ${q(c.sni)}`,
      '    client-fingerprint: chrome', '    skip-cert-verify: false'].join('\n');
  }
  const lines = [...common, `    uuid: ${q(c.uuid)}`, `    network: ${c.network}`, '    tls: true',
    '    udp: true', `    servername: ${q(c.sni)}`, '    client-fingerprint: chrome'];
  if (c.security === 'reality') {
    lines.push('    reality-opts:', `      public-key: ${q(c.pbk)}`, `      short-id: ${q(c.sid)}`, '    flow: xtls-rprx-vision');
  } else {
    lines.push('    skip-cert-verify: false', '    ws-opts:', `      path: ${q(c.path)}`, '      headers:', `        Host: ${q(c.host)}`);
  }
  return lines.join('\n');
}
function clash(c) {
...
export default {
  async fetch(request, env) {
    ...
    const headers = new Headers({
      ...
      'Content-Disposition': isClash ? `attachment; filename="${downloadName(env)}"` : 'inline',
      'Content-Type': isClash ? 'text/yaml; charset=UTF-8' : 'text/plain; charset=UTF-8'
    });
    return new Response(isClash ? clash(CONFIG) : b64(link(CONFIG)), {headers});
  }
};
EOF
