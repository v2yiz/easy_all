...
#
# This Profile is intentionally loaded only by easy_all's fourth installation
...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
    printf 'websocket-gcore.sh 是 easy_all 的 Gcore CDN Profile；请使用：easy_all install\n' >&2
    exit 2
fi

readonly WEBSOCKET_GCORE_PROFILE_ROOT="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"
readonly XHTTP_PROFILE_ROOT="${WEBSOCKET_GCORE_PROFILE_ROOT}/../lib"
readonly XHTTP_PROFILE_FILE="${WEBSOCKET_GCORE_PROFILE_ROOT}/websocket-gcore.sh"
...
readonly GCORE_WS_NGINX_TIMEOUT="1h"
...
gcore_ensure_resource() {
    ...
    if [[ -n "${GCORE_CDN_RESOURCE_ID:-}" ]]; then
        ...
        jq -e --arg domain "${VLESS_CDN_DOMAIN}" \
            --arg name "easy_all websocket ${VLESS_CDN_DOMAIN}" '
            (.cname | rtrimstr(".") | ascii_downcase) == $domain and .name == $name
        ' <<<"${current}" >/dev/null \
    ...
    if ((count == 1)); then
        jq -e --arg expected "easy_all websocket ${VLESS_CDN_DOMAIN}" \
            '.[0].name == $expected' <<<"${matches}" >/dev/null \
...
load_state() {
    ...
    local -a variables=(
        PROTOCOL CDN_PROVIDER GCORE_CDN_ENDPOINT_MODE
        CDN_CLIENT_IP_FAMILY WS_NODE_NAME VLESS_UUID
        VLESS_CDN_DOMAIN SUBSCRIPTION_DOMAIN WS_PATH
        ...
        XRAY_WS_LOOPBACK_PORT ORIGIN_HEADER_SECRET ALLOWED_TOKENS SUB_DOWNLOAD_NAME
    ...
    AWS_ORIGIN_DOMAIN=${GCORE_ORIGIN_DOMAIN}
    configure_cdn_traffic_protection
    [[ -n "${XHTTP_NODE_NAME:-}" ]] || die "状态缺少 WS_NODE_NAME；请重新安装"
...
xhttp_render_xray_config() {
    ...
    jq -n --argjson ws_port "${XRAY_XHTTP_LOOPBACK_PORT}" \
        --argjson clients "${clients}" --argjson stats_enabled "${stats_enabled}" \
        --arg ws_path "${XHTTP_PATH}" --arg ws_host "${VLESS_CDN_DOMAIN}" \
        --argjson heartbeat "${GCORE_WS_HEARTBEAT_PERIOD}" \
        ...
        {log:{loglevel:"warning"},
         inbounds:[{tag:"vless-ws-in",listen:"127.0.0.1",port:$ws_port,protocol:"vless",
          settings:{clients:$clients,decryption:"none"},
          streamSettings:{network:"ws",security:"none",sockopt:$inbound_sockopt,
            wsSettings:{host:$ws_host,path:$ws_path,acceptProxyProtocol:false,heartbeatPeriod:$heartbeat}},
...
EOF
    ...
    location = ${XHTTP_PATH} {
        ...
        proxy_read_timeout ${GCORE_WS_NGINX_TIMEOUT};
        proxy_send_timeout ${GCORE_WS_NGINX_TIMEOUT};
...
build_vless_xhttp_link() {
    ...
    local client_path="${XHTTP_PATH}?ed=${GCORE_WS_EARLY_DATA}"
    printf 'vless://%s@%s:443?encryption=none&security=tls&type=ws&sni=%s&fp=chrome&alpn=http%%2F1.1&host=%s&path=%s&packetEncoding=xudp#%s' \
        ...
        "$(uri_encode "${client_path}")" "$(uri_encode "${node_name}")"
}
...
build_mihomo_node_for_endpoint() {
    ...
    jq -nr --arg name "${node_name}" --arg server "${server}" \
        ...
        --argjson early_data "${GCORE_WS_EARLY_DATA}" '
        "  - name: \($name|@json)\n    type: vless\n    server: \($server|@json)\n    port: 443\n" +
        "    uuid: \($uuid|@json)\n    network: ws\n    tls: true\n    udp: true\n" +
        "    skip-cert-verify: false\n    servername: \($host|@json)\n    client-fingerprint: chrome\n" +
        "    packet-encoding: xudp\n    ip-version: \($ip_version)\n    alpn:\n      - http/1.1\n" +
        "    ws-opts:\n      path: \($path|@json)\n      headers:\n        Host: \($host|@json)\n" +
        "      max-early-data: \($early_data)\n      early-data-header-name: Sec-WebSocket-Protocol\n"'
}

validate_cdn_candidate_transport() {
    local ip=$1 status
    status=$(curl -sS --http1.1 --connect-timeout 4 --max-time 3 --noproxy '*' \
        --resolve "${VLESS_CDN_DOMAIN}:443:${ip}" -o /dev/null -w '%{http_code}' \
        -H "Host: ${VLESS_CDN_DOMAIN}" \
        -H 'Connection: Upgrade' -H 'Upgrade: websocket' \
        -H 'Sec-WebSocket-Version: 13' \
        -H 'Sec-WebSocket-Key: ZWFzeV9hbGxfd3NfdGVzdA==' \
        "https://${VLESS_CDN_DOMAIN}${XHTTP_PATH}" 2>/dev/null || true)
    [[ "${status}" == "101" ]]
}
...
validate_protocol_runtime() {
    ...
    for attempt in 1 2 3 4 5; do
        if systemctl is-active --quiet "${XRAY_SERVICE}" \
            ...
            response=$(curl -fsS --proto '=https' --noproxy '*' \
                --cert "${GCORE_CLIENT_CERT_FILE}" --key "${GCORE_CLIENT_KEY_FILE}" \
                --resolve "${AWS_ORIGIN_DOMAIN}:443:127.0.0.1" \
                -H "X-Easy-All-Origin-Key: ${ORIGIN_HEADER_SECRET}" \
                "https://${AWS_ORIGIN_DOMAIN}/easy_all-health" 2>/dev/null || true)
            status=$(curl -sS --http1.1 --connect-timeout 3 --max-time 3 --noproxy '*' \
                --cert "${GCORE_CLIENT_CERT_FILE}" --key "${GCORE_CLIENT_KEY_FILE}" \
                --resolve "${AWS_ORIGIN_DOMAIN}:443:127.0.0.1" \
                -H "X-Easy-All-Origin-Key: ${ORIGIN_HEADER_SECRET}" \
                -H 'Connection: Upgrade' -H 'Upgrade: websocket' \
                ...
                "https://${AWS_ORIGIN_DOMAIN}${XHTTP_PATH}" 2>/dev/null || true)
            if [[ "${response}" == "easy_all ok" && "${status}" == "101" ]]; then
                validate_quota_api
                return 0
            fi
        fi
...
xhttp_renew_origin_certificate() {
    ...
    run_acme --renew -d "${AWS_ORIGIN_DOMAIN}" --ecc --force \
        || die "源站证书续期失败"
