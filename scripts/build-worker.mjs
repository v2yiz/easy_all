...
export async function buildWorker({
    ...
    intranetProxyDomain = process.env.INTRANET_PROXY_DOMAIN || '',
    now = Date.now(),
} = {}) {
    let config;
    ...
    let subscriptionTemplate = template;
    if (intranetProxyDomain) {
        const routed = spawnSync('bash', [resolve(root, 'lib/mihomo-template.sh'), templatePath, intranetProxyDomain], { encoding: 'utf8' });
        if (routed.status !== 0) throw new Error('Invalid intranet proxy domain or template');
        subscriptionTemplate = routed.stdout;
    }
    ...
    const content = '// Generated by npm run build:worker. Contains private configuration; do not commit.\n'
        ...
        + `const MIHOMO_TEMPLATE = ${JSON.stringify(subscriptionTemplate)};\n\n` + source;
