...
- `config.local.json`：用户 token、Reality 节点、VPS CF 订阅地址、CF 备用节点、XFLASH 订阅地址。Reality 端口继续按北京时间每三小时轮换；只有节点域名 AAAA 已验证指向双栈 VPS 时，才把该 Reality 节点的 `ipVersion` 设为 `dual`。
