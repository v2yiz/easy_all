...
readonly ENTRY_SCRIPT_FILE="${EASY_ALL_ENTRY_SCRIPT:-}"
...
cleanup() {
    ...
    case "${EASY_ALL_DOWNLOADED_CORE_FILE:-}" in
    /tmp/easy-all-core.* | /tmp/easy-cmcc-core.*)
        rm -f -- "${EASY_ALL_DOWNLOADED_CORE_FILE}"
        ;;
    esac
}
trap cleanup EXIT
...
register_easy_all_command() {
    local destination="${COMMAND_INSTALL_DIR}/${ENTRY_COMMAND_NAME}.sh"
    require_root
    ...
    success "已注册命令：${COMMAND_PATH}（启动时下载共享安装核心）"
}
...
用法: ${ENTRY_COMMAND_NAME} [命令]
  ...
  install       安装 RackNerd / 上海移动专用 VLESS XHTTP + WSS
  update        刷新 RN 双传输、Nginx 与独立 easy-cmcc Worker
  update-sub    刷新服务端与订阅
  show|subscription|status|update-core|renew-cert|uninstall

该入口固定使用 VLESS XHTTP + WSS，Worker 名称固定为 easy-cmcc。
BWG/VM 的 Reality 或 AnyTLS 请使用 easy_all。
EOF
        return 0
    fi
    cat <<EOF
...
main() {
    if [[ "${EASY_ALL_PROFILE}" == "cmcc" && "${1:-install}" == "switch" ]]; then
